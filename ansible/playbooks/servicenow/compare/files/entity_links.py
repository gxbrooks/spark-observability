...
# ServiceNow table for logical entity types used in compare reports.
SN_ENTITY_TABLES = {
...
def normalize_dynatrace_ui_base(tenant_or_api_url: str) -> str:
    """Dynatrace entity navigation uses the *.live.dynatrace.com UI host."""
...
def sn_record_url(instance_url: str, table: str, sys_id: str) -> str:
    """ServiceNow record URL via nav_to.do (works across Classic and Workspace UIs).

    See ServiceNow navigation-by-URL: nav_to.do?uri=<table>.do?sys_id=<sys_id>
    """
...
def dt_entity_url(tenant_or_api_url: str, entity_id: str) -> str:
    """Dynatrace entity navigation URL.

    Format documented by Dynatrace:
    https://<environment>.live.dynatrace.com/ui/nav/<ENTITY_ID>
    """
...
def resolve_dynatrace_link_base(instance: dict, meta: dict | None = None) -> str:
    """Prefer explicit UI base (from DT_API_URL); else normalize tenant URL."""
    meta = meta or {}
    inst = instance or {}
...
def link_for_entity(entity: dict, sn_url: str, dt_url: str) -> str:
    """Resolve a single deep link from an entity dict."""
    if not entity:
        return ""
...
def enrich_entity(entity: dict | None, sn_url: str, dt_url: str) -> dict | None:
    """Add url and optional sample_urls to an entity dict (in place copy)."""
    if not entity:
        return entity
    out = dict(entity)
...
def enrich_row(row: dict, sn_url: str, dt_url: str) -> dict:
    """Add ServiceNow and/or Dynatrace URLs to inventory/diff rows."""
    out = dict(row)
