...
4. Writes **`tmp/compare/<timestamp>/compare_data.json`** (raw), **`compare_report.json`** (annotated findings with deep links), and per-scope **`{scope_unit_id}.json`**.

**Report output:** `compare_report.json` contains prescriptive **findings** (severity, category, observation, resolution steps, ansible commands, doc links) and **inventory** sections. Each entity includes a clickable **`url`**: ServiceNow `nav_to.do?uri=<table>.do?sys_id=...`, Dynatrace `{live-tenant}/ui/nav/{entityId}` (see [Dynatrace entity navigation](https://www.dynatrace.com/news/blog/massively-automate-enterprise-operations-using-dynatrace-information-discovered-with-the-monitored-entities-api-v2/)).
...
| 2026-06-24 | Added dynatrace-correlation.yaml and prescriptive diagnostics in compare report |
