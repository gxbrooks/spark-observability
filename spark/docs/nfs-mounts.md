...
| `/mnt/spark/client-logs/<instance>/` | *(under local logs root)* | Same local store as above | No | Yes — client driver on that host |
...
| `/mnt/spark/client-logs/<instance>/` | `spark` or driver user | `spark` | `755` | Directory must be traversable by `dtuser` |
