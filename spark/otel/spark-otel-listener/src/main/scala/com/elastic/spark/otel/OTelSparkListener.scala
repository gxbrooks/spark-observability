...
class OTelSparkListener extends SparkListener {
  ...
  override def onApplicationStart(applicationStart: SparkListenerApplicationStart): Unit = {
    try {
      ...
      val span = tracer.spanBuilder(s"spark.application.$appName")
        ...
        .setStartTimestamp(startTime * 1000000) // Convert ms to ns
  ...
  override def onApplicationEnd(applicationEnd: SparkListenerApplicationEnd): Unit = {
    try {
      ...
      applicationSpan.values.foreach { span =>
        ...
        span.end(endTime * 1000000) // Convert ms to ns
      }
  ...
  override def onJobStart(jobStart: SparkListenerJobStart): Unit = {
    try {
      ...
      val spanBuilder = tracer.spanBuilder(s"spark.job.$jobId")
        ...
        .setStartTimestamp(time * 1000000)
  ...
  override def onStageSubmitted(stageSubmitted: SparkListenerStageSubmitted): Unit = {
    try {
      ...
      // Try to get parent job span
      val parentSpan = if (stageInfo.jobIds.nonEmpty) {
        jobSpans.get(stageInfo.jobIds.head)
      } else {
        None
      }
      ...
      val spanBuilder = tracer.spanBuilder(s"spark.stage.$stageId")
        .setSpanKind(SpanKind.INTERNAL)
        .setStartTimestamp(submissionTime * 1000000)
  ...
  override def onStageCompleted(stageCompleted: SparkListenerStageCompleted): Unit = {
    try {
      ...
      stageSpans.remove(stageId).foreach { span =>
        ...
        span.end(completionTime * 1000000)
      }
    } catch {
      case e: Exception =>
    ...
    if (taskEnd.reason != Success) {
      try {
        ...
        val spanBuilder = tracer.spanBuilder(s"spark.task.${taskInfo.taskId}")
          .setSpanKind(SpanKind.INTERNAL)
          .setStartTimestamp(taskInfo.launchTime * 1000000)
        ...
        span.end(taskInfo.finishTime * 1000000)
      } catch {
        case e: Exception =>
