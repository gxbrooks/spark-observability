#!/bin/bash
#
# Bootstrap wrapper for vars-grid.py
# Uses system Python3 to avoid circular dependencies
# Safe to run before any environment is set up
#
# This script ensures vars-grid.py can run even when:
# - No virtual environment exists
# - Environment variables are not set
# - Python version is not yet determined
#
# Usage: Same as vars-grid.py
#   ./vars-grid.sh                    # Generate grid for all contexts
#   ./vars-grid.sh --missing          # Show only missing contexts
#   ./vars-grid.sh --summary          # Show summary statistics
#   ./vars-grid.sh -c devops,spark-client  # Show grid for specific contexts
...
# Run vars-grid.py with system Python
cd "$REPO_ROOT"
exec "$PYTHON_CMD" "$SCRIPT_DIR/vars-grid.py" "$@"
