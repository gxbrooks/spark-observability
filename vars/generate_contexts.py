...
def write_ansible_vars(vars_dict, filename, context_name='ansible'):
    ...
    try:
        with open(filename, 'w') as f:
            ...
            # The `nfs` context shares the ansible_vars writer type, but it must not require Spark
            # registry settings (and it emits a different variable surface).
            if context_name == 'nfs':
                f.write('# NFS configuration\n')
                if 'NFS_SERVER' in vars_dict:
                    f.write(f'nfs_server: "{vars_dict["NFS_SERVER"]}"\n')
                if 'SPARK_MOUNT_BASE' in vars_dict:
                    f.write(f'spark_mount_base: "{vars_dict["SPARK_MOUNT_BASE"]}"\n')
                if 'SPARK_DATA_MOUNT' in vars_dict:
                    f.write(f'spark_data_mount: "{vars_dict["SPARK_DATA_MOUNT"]}"\n')
                if 'SPARK_DATA_SOURCE' in vars_dict:
                    f.write(f'spark_data_source: "{vars_dict["SPARK_DATA_SOURCE"]}"\n')
                if 'NFS_SPARK_DATA_EXPORT' in vars_dict:
                    f.write(f'nfs_spark_data_export: "{vars_dict["NFS_SPARK_DATA_EXPORT"]}"\n')
                f.write('\n')
                return True
            ...
            # Write registry configuration (Spark image build + registry host from variables.yaml)
            f.write('# Registry and image configuration (local registry on observability host)\n')
            registry_host = vars_dict.get('SPARK_REGISTRY_HOST', '')
            if not registry_host:
                raise ValueError("SPARK_REGISTRY_HOST is required for ansible context generation")
...
def main(requested_contexts=None, force=False, verbose=False):
    ...
    for context in contexts_to_generate:
        ...
        elif writer_func_name == 'write_ansible_vars':
            success = writer_func(vars_dict, str(output_path), context_name)
        else:
