...
def write_ansible_vars(vars_dict, filename, context_name='ansible', variables=None):
    """Write context-tagged variables to Ansible vars YAML (order + section from variables.yaml)."""
    try:
        with open(filename, 'w') as f:
            ...
            for key, value in vars_dict.items():
                if variables and isinstance(variables.get(key), dict):
                    section = variables[key].get('section', '').strip()
...
def main(requested_contexts=None, force=False, verbose=False):
    ...
    for context in contexts_to_generate:
        ...
        if writer_func_name == 'write_systemd_env':
            ...
            success = writer_func(vars_dict, str(output_path), context_name, variables)
        else:
