...
def _render_inline_template(template_str, emitted_vars, source_vars):
    ...
    def repl(match):
        ...
        if var_name in emitted_vars:
            return str(emitted_vars[var_name])
        if var_name in source_vars:
            return str(source_vars[var_name])
        return match.group(0)

    return re.sub(r'\{\{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\}\}', repl, template_str)


def write_ansible_vars(vars_dict, filename, context_name='ansible', context_spec=None):
    """Write variables to Ansible variables YAML file with structured formatting"""
    try:
        render_spec = (context_spec or {}).get('ansible_render', {})
        ...
        with open(filename, 'w') as f:
            ...
            # Declarative rendering from contexts.yaml metadata.
            if sections:
                for section in sections:
                    ...
                    for entry in section.get('entries', []):
                        ...
                        if 'source' in entry:
                            ...
                            value = vars_dict.get(source_key)
            ...
            # Fallback: write all context variables in straightforward YAML form.
            for key, value in vars_dict.items():
                f.write(f'{key}: {_yaml_double_quote(value)}\n')
            f.write('\n')
        return True
    except IOError as e:
        print(f"Error writing to {filename}: {e}")
...
def main(requested_contexts=None, force=False, verbose=False):
    ...
    for context in contexts_to_generate:
        ...
        if writer_func_name == 'write_systemd_env':
            ...
            success = writer_func(vars_dict, str(output_path), context_name, context)
        else:
