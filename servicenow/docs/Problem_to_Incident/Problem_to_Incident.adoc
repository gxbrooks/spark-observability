...
digraph end_to_end_flow {
  ...
  edge [fontname="Noto Sans" fontsize=8 labelangle=0 labeldistance=2.0 arrowsize=0.65]
...
|0
|ServiceNow CMDB / SGC
|Application Services, workload CIs, tags, `sys_object_source`; pattern-specific CMDB model (<<step-0-cmdb-model>>)
...
|5
|ServiceNow ITSM
|`incident.cmdb_ci` = Application Service (<<step-5-service-side>> or <<step-5-client-side>>)
|===
...
|3
|Event Management rules mark SGO-Dynatrace log events **Ready** when `cmdb_ci` is populated (<<step-5-em-rules>>)
...
|2
|Client drivers write under `/mnt/spark/logs/spark-client/<instance>/` (<<step-1-log-emission-client>>)

|3
|Dynatrace custom log source and OpenPipeline list **spark-client** paths **before** generic `/mnt/spark/logs/*/`

|4
|Incident automation implements `resolveFromSparkClientLogPath()` (<<step-5-client-side>>)
|===
...
**Out of scope:** host CI binding without pod rebind, process group CI, tag-only lookup without a Contains edge. Docker workloads: link:docker_model.md[CMDB model and correlation patterns].
...
. Look up Application Service **`Spark Client`** by **`name`** (see <<csdm-identifier-field-mapping>>).
