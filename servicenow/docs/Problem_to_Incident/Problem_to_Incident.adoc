...
The flow is vendor-specific only where the platforms require it (Dynatrace OpenPipeline, Service Graph Connector, CSDM). Application names, host names, and tenant identifiers in examples reflect the **brooks-lab Spark** reference implementation unless noted otherwise. Implementors may automate these steps with Ansible, Terraform, REST APIs, or manual configuration.
...
|**Spark Service-Side Log Pattern** — traverse **Contains::Contained by** from `cmdb_ci_kubernetes_pod` to Application Service

|Client-side
|PySpark driver JVMs on any host (lab servers today; enterprise laptops and VMs tomorrow)
|**Spark Client-Side Log Pattern** — fixed path segment `/logs/spark-client/` → **Spark Client** Application Service (`service_mapping: manual`)
|===
...
digraph end_to_end_flow {
  ...
  node [shape=box style="rounded,filled" fillcolor="#eef6ff" fontname="Noto Sans" fontsize=9 width=2.2 fixedsize=true height=0.28]
  edge [fontname="Noto Sans" fontsize=8 decorate=true labelangle=0 labeldistance=2.8 arrowsize=0.65]

  App [label="Application JVM"]
  Log [label="application.log"]
  OA [label="Dynatrace OneAgent"]
  OP [label="OpenPipeline"]
  ...
  INC -> AS [label="Service-Side or Client-Side correlation"]
}
----
...
|0
|ServiceNow CMDB / SGC
|Application Services, workload CIs, tags, `sys_object_source`; pattern-specific CMDB model (<<step-0-cmdb-model,CMDB model>>)

|1
|Application / Log4j
|ERROR/WARN on a path that encodes workload identity (pod name or `spark-client`)
...
|`em_event` / `em_alert` bind to infrastructure CI

|5
|ServiceNow ITSM
|`incident.cmdb_ci` = Application Service (<<step-5-service-side,Service-side pattern>> or <<step-5-client-side,Client-side pattern>>)
|===
...
Before log problems can bind to infrastructure CIs and incidents to an **Application Service**, ServiceNow must contain the CSDM hierarchy, pattern-specific CMDB models (<<step-0-cmdb-model,CMDB model>>), and incident automation (<<step-5-incident-correlation,Incident correlation>>).
...
image::images/spark-csdm-cmdb-model.svg[Spark Service-Side Log Pattern — CMDB and CSDM correlation model,width=100%,align=center]
...
.Enterprise client CMDB model
[graphviz, Enterprise_client_CMDB_model]
----
digraph enterprise_client {
  graph [rankdir=TB splines=true nodesep=0.4 fontname="Noto Sans" fontsize=12]
  node [shape=box style="rounded,filled" fillcolor="#f8f8f8" fontname="Noto Sans" fontsize=11]

  BA [label="cmdb_ci_business_app\nData and Analytic Services"]
  BS [label="cmdb_ci_business_service\nApache Spark"]
  AS [label="cmdb_ci_service_discovered\nSpark Client\nservice_mapping: manual\nservice_status: operational"]
  SM [label="cmdb_ci_service_discovered\nSpark Master"]

  BA -> BS
  BS -> AS
  BS -> SM
  AS -> SM [label="depends_on"]

  LAP [label="Developer laptop\n(out of CMDB scope)" style="dashed,filled" fillcolor="#fff8e8"]
  LAB [label="Lab3 host process\n(optional)"]
  LAP -> AS [style=dashed label="no Contains edge"]
  LAB -> AS [style=dashed label="no required edge"]
}
----
