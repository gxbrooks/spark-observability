...
== Purpose
Large incident resolution times can impact business operations and customer satisfaction. One way to reduce resolution times for incidents is to bring those critical problems directly to attention of the appropriate application operations team in a timely manner. This requires seamlessly bridging observability processes to IT service management processes. This documents shows how to integrate observability to IT service management into a seamless process. 

To illustrate this integration we use Apache Spark as the application of interest along with Dynatrace as the observability platform and ServiceNow as the IT service management platform. Spark is a popular open-source distributed computing system that is used for large-scale data processing and analytics. It is used in a wide range of industries, including finance, healthcare, and retail. Dynatrace is a popular observability platform that is used to monitor and troubleshoot applications. ServiceNow is the market leader in IT service management  that is used to manage IT services and incidents.

We call this class of observability-to-ITSM processes, which starts in Dynatace as a problem and ends in ServiceNow as a resolved incident, **Problem to Incident**. 
...
The *Log to Incident* process is subclass of the Problem to Incident process that takes critical issues (problems) that are noted in an application log file and raise an incident in ServicNow. This alows for prompt attention to be brought to the problem by the application operations team.

Not all log file entries are critical enough to justify an incident. Observabilty engineers should work with application developers to identify which log entries are critical and warrant an incident. Otherwise, operations teams could be flooded with incidents that are not critical and consume time to resolve and close. Observability engineers and application developers should also work to together on the structure and format of the log entries to ensure that key relevant information about the application is communicated to the application operations team to make it easier for the application team to resolve. 

For the purposes of this implementation, we designated Spark WARN and ERROR log messages as critical enough to justify an incident. In a real-world implementation, only ERROR log messages would be designated as critical enough to justify an incident. Even then critical log entries shoujd only be designated after discussion with the application operations and development teams. 

=== End-to-end flow
The figure below illustrates the end-to-end flow of the Log to Incident process starting with the application JVM writing to the application log file and ending with the incident being created in ServiceNow.
...
image::images/spark-architecture.svg[Spark client-server architecture — Master, Workers, client driver, and History Server,width=75%,align=center]
...
image::images/spark-servicenow-model-service-side.svg[Spark Service-Side model,width=100%,align=center]
...
image::images/spark.servicenow-model-client-side.svg[Spark Service-Side model,width=100%,align=center]
...
. OpenPipeline emits separate Davis events per log line with distinct `event.name` values (`… on spark-client` vs `… on spark-master-0`).
...
. `em-alert-create-k8s-log-incident` scans for `/logs/spark-client/` **first**; if absent from the bundled text ordering, **Service-side** correlation may win and set **`incident.cmdb_ci`** to **Spark Master** even when the operator intended **Spark Client**.
. Event Management may also pre-link alerts to an existing open incident (`INC0013763`) before the business rule corrects Application Service assignment.
