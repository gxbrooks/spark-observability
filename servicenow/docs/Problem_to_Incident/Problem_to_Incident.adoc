...
|6
|Application workloads that emit ERROR/WARN logs run as **Kubernetes pods** with `metadata.name` equal to the log directory segment (Step 1), **or** as **client-mode PySpark drivers** writing under `/mnt/spark/logs/spark-client/<instance>/` (<<step-5-spark-client-path>>)

|7
|Spark Client AS `service_status=operational` (bodyless host AS; see <<step-5-spark-client-path>>)
...
==== Spark client-mode driver (chapter and ad-hoc jobs)

PySpark **client-mode** drivers run as a **Linux host process** on Lab1–Lab3 (not a Kubernetes pod). Each driver JVM **must** write to its own subdirectory so concurrent drivers do not share the same `spark-app.log` RollingFile target.

| Attribute | Source | Consumed by |
| Timestamp, level, logger, message | Log4j PatternLayout | Dynatrace `loglevel`, `content`; ServiceNow `em_event.description` |
| Fixed AS path segment `spark-client` | `run-chapters.sh` sets `SPARK_LOG_DIR=/mnt/spark/logs/spark-client/<instance>` | OpenPipeline Davis matcher; incident BR path lookup (<<step-5-spark-client-path>>) |
| Per-driver instance directory | `{hostname}-{pid}` (override with `SPARK_DRIVER_INSTANCE`) | OneAgent custom log source `${1}` enrichment → `spark.client.instance` |
| Optional process tag | `DT_TAGS=servicenow.io/application-service-identifier=spark-client` | Dynatrace process-group problems only; **not** required for log-path incidents |
...
**Key fields:** **`path`** patterns **must** align with Log4j **`fileName`** / **`filePattern`** (<<spec-2-4-2-1>>) and OpenPipeline **`log.source`** matchers (<<spec-dt-openpipeline>>). **Pod** paths use segment = `cmdb_ci_kubernetes_pod.name`. **Client-mode** paths use `/mnt/spark/logs/spark-client/<instance>/` (<<step-5-spark-client-path>>) with enrichment `spark.client.as_identifier` and `spark.client.instance`. OpenPipeline **drops** legacy `*-chapter` paths parsed as pod names. **`enrichment`** attributes **`k8s.log.path`** (`${0}`) and **`k8s.pod_name`** or **`spark.client.instance`** (`${1}`) **must** be set on wildcard paths — list **more specific** patterns (spark-client) **before** generic `/mnt/spark/logs/*/`.
...
**Out of scope for Pattern A:** host CI binding without pod rebind, process group CI, tag-only lookup without a Contains edge. Docker workloads are documented separately in link:docker_model.md[CMDB model and correlation patterns].

==== Incident correlation (Spark client — log path to Application Service)
[[step-5-spark-client-path]]

Client-mode driver logs **do not** correspond to a `cmdb_ci_kubernetes_pod`. **`em_alert.cmdb_ci`** may remain the SGO-imported **`cmdb_ci_linux_server`** (HOST entity from the problem). Incident automation **must not** depend on alert CI or Contains edges for client-mode logs.

.Incident Application Service correlation (Spark client log path)
[graphviz, Incident_Application_Service_correlation_spark_client]
----
...
. Resolution algorithm (Spark client)
. Scan `em_alert.description`, `resource`, and `node` for the substring `/logs/spark-client/`.
. When present, look up Application Service **`Spark Client`** (CSDM `identifier: spark-client`; see <<csdm-identifier-field-mapping>>).
. Set **`incident.cmdb_ci`** to that Application Service sys_id.
. **`K8sLogPodCiBind`** skips path segment `spark-client` when parsing pod names so client paths are not mis-bound to a pod CI.

**Scenario:** WARN or ERROR from a chapter driver in `/mnt/spark/logs/spark-client/lab3-par-a-557204/spark-app.log`. Davis **`event.name`** = `Application log {loglevel} on spark-client`. ServiceNow creates **`incident.cmdb_ci`** = **Spark Client** Application Service regardless of whether **`em_alert.cmdb_ci`** is Lab3 HOST or empty.
...
|**incident.cmdb_ci** (path-based lookup)

|cmdb_ci_linux_server — Lab3
|**em_alert.cmdb_ci** when Davis binds problem to HOST (acceptable; not used for incident CI)

|Log path segment `spark-client`
|Fixed Application Service key in path; **not** a pod name

|Per-driver segment `<instance>` (e.g. `lab3-par-a-557204`)
|Disambiguates concurrent drivers; **not** used for AS lookup
|===

==== CSDM `identifier` vs CMDB Application Service lookup
[[csdm-identifier-field-mapping]]

CSDM **`identifier`** in `*.csdm.yaml` is the **logical machine key** for an Application Service. It serves **two related but distinct** purposes on the ServiceNow instance:

[cols="30%,35%,35%",options="header"]
|===
|Purpose |Mechanism |Queryable field on `cmdb_ci_service_discovered`

|Tag-based Service Mapping population
| `csdm/deploy.yml` → `configure_tag_based_sm.yml` POSTs **`tag_list`** to Service Mapping Operations REST (`populate_tags`) with key **`servicenow.io/application-service-identifier`** = CSDM `identifier`
| **`tag_list`** (Service Mapping filter JSON; often omitted from Table API responses)

|Documentation / comparator / Pattern B (Docker)
| Comparator and docker_model reference **`identifier`** as the AS machine key matching workload tags
| **`identifier`** column — **not populated** on optimizincdemo1 for CSDM-deployed rows (REST PATCH accepts the field but GET omits it; encoded query `identifier=spark-client` does not return Spark Client)

|Display
| CSDM **`name`** (e.g. `Spark Client`)
| **`name`** — reliable for GlideRecord lookup
|===

**Implication for Log-to-Incident:** `ResolveApplicationService.resolveFromSparkClientLogPath()` maps the fixed path segment **`spark-client`** to Application Service **`Spark Client`** by **`name`** (with optional `identifier` query when the column is populated on other instances). Do **not** assume `cmdb_ci_service_discovered.identifier` is set by `csdm/deploy.yml` on every platform release.

**Verification:** Spark Client AS **must** have `service_status=operational` (bodyless host AS — promoted by `csdm/deploy.yml` → `ensure_host_as_operational.yml`). Without operational status, ServiceNow rejects `incident.cmdb_ci` assignment to the Application Service.
...
|Pattern A: `cmdb_rel_ci` Contains::Contained by from pod CI to Application Service. Spark client: `resolveFromSparkClientLogPath()` when alert text contains `/logs/spark-client/` → Application Service **Spark Client**
...
|**After** insert/update on `em_alert`; order **5020**; tries spark-client path first, else Pattern A Contains; creates or correlates incident, sets **`em_alert.incident`**
