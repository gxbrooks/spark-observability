...
Client Mode logs use a **separate tree** from Cluster Mode: `/mnt/spark/client-logs/<instance>/` versus `/mnt/spark/logs/<pod>/`. A short path-prefix comparison differentiates the modes. The **instance** path element defaults to the wrapper PID of the Spark Driver; parallel drivers on one host set **SPARK_DRIVER_INSTANCE**. OpenPipeline sets **`spark.mode=Client`** and **`spark.instance=<host.name>:<instance>`**.
...
----
/mnt/spark/
├── client-logs/                         # Client mode (node-local)
│   ├── 12345/                           # default: wrapper PID
│   ├── par-a/                           # SPARK_DRIVER_INSTANCE override
│   └── par-b/
└── logs/                                # Cluster mode pods only
    ├── spark-master-0/
    └── spark-worker-…
----
...
Client-side (`spark/conf/log4j2-client.properties`): console + RollingFile. `run-chapters.sh` sets `SPARK_LOG_DIR=/mnt/spark/client-logs/<instance>/` and `-Dlog4j2.configurationFile=…/log4j2-client.properties`.
    ...
    "custom-log-source": {
      ...
      "values-and-enrichment": [
        ...
        },
        {
          ...
          "enrichment": [
            { "type": "attribute", "key": "spark.log.path", "value": "${0}" },
            { "type": "attribute", "key": "spark.mode", "value": "Cluster" },
            { "type": "attribute", "key": "spark.pod_identifier", "value": "${1}" }
          ]
        },
        {
          "path": "/mnt/spark/logs/*/spark-app-*.log",
...
.OneAgent and custom log source metadata fields
...
|Custom log source `${1}` (Cluster paths only)
|Pod directory under `/mnt/spark/logs/` (e.g. `spark-master-0`); not a Dynatrace entity id

|`spark.as_identifier`
|Custom log source (Client paths)
|`spark-client` (Application Service `identifier` for ServiceNow bind)
|===
    ...
    "routingEntries": [
      {
        ...
        "matcher": "matchesValue(log.source, \"/mnt/spark/logs/*\") OR matchesValue(log.source, \"/mnt/spark/client-logs/*\")",
...
This rule routes both Cluster (`/mnt/spark/logs/*`) and Client (`/mnt/spark/client-logs/*`) paths to the Spark log processing OpenPipeline.
...
.OpenPipeline entry fields by Spark mode
[cols="16%,42%,42%",options="header"]
|===
|Field Name|Cluster Mode|Client Mode

|spark.mode
|`Cluster` (from custom source)
|`Client` (from custom source)

|spark.pod_identifier
|directory name under `/mnt/spark/logs/` (e.g. `spark-master-0`)
|unset
...
|dt.source_entity
|HOST (OneAgent file tail; in Spark Observability MZ)
|HOST (OneAgent file tail; in MZ). SN binds via `spark.as_identifier`, not HOST
|===

The Spark OpenPipeline processors are defined in <<lst-openpipeline-processing>>, below. There are four processor rules:

. **`tag-pipeline-custom-id`** - for easy Grail filtering stamps the OpenPipeline customId in a `pipeline` field on every log record 

. **`compose-client-instance`** - defines the `spark.instance` field for client log entries from the `host.name` and `spark.driver.instance` fields.
...
Once the metadata fields are defined in the processors, we are ready to create Davis problems for those "critical" log entries with a log level of WARN or ERROR. There are two Davis processors: one for Client Mode log entries and one for cluster Mode log entries. There is a third Davis processor for log entries that do not have a `spark.mode` defined. All Spark log entries should have a `spark.mode` defined, but we include this as a "fail-visible" Davis event to help with debugging.
...
|`event.type`
|is always `ERROR_EVENT` (Dynatrace Settings API fixed enum)

|`spark.event_kind`
|is always `CRITICAL_LOG_EVENT` (lab semantic label on Davis/custom property)

|`event.name`
|`Spark critical {loglevel} error at {spark.log.class}:{spark.log.line}`

|`event.description`
|includes `{spark.log.class}:{spark.log.line}`, path, and `spark.as_identifier=` or `spark.pod_identifier=` for SN bind

|`dt.source_entity`
|HOST (OneAgent file tail; in MZ). SN binds via `spark.as_identifier` / `spark.pod_identifier`

|`spark.as_identifier` / `spark.pod_identifier`
|Client AS identifier or Cluster pod name (stamped into description + properties)

|`event.unique_identifier`
|mode-scoped critical error signature (`Spark Client|Cluster Critical …`)

|`pipeline`
|`spark-lab-log-alerts` (OpenPipeline customId; stamped in processing)

|`dt.davis.is_merging_allowed`
|`false` to prevent merging of multipleDavis events into the same problem

|dt.davis.event_timeout
|`15m` which is probably unnecessary as we are not using problem merge
...
.Example problem notification payload
[source,json]
----
{
    ...
    "rankedEvents": [
      {
        ...
        "eventDescription": "Application ERROR on /var/log/myapp/myapp-worker-7d4f9b-xk2lm/application.log spark.pod_identifier=myapp-worker-7d4f9b-xk2lm: 2026-06-27 14:53:46 ERROR Worker:264 - All masters are unresponsive! Giving up."
      }
    ],
...
. After a test problem fires, confirm **`em_event.source=SGO-Dynatrace`** and **`em_event.cmdb_ci`** is populated (pod CI for Cluster after `K8sLogPodCiBind`; Spark Client AS for Client).
. If Cluster **`cmdb_ci`** remains a Linux server instead of a pod, return to <<step-2-dynatrace,Phase 3 — Dynatrace>> and verify **`spark.pod_identifier`** enrichment in the custom log source (<<spec-dt-openpipeline,OpenPipeline specification>>), then the entity bind rules.
...
. Deploy or verify the **K8s log pod bind** business rules (<<step-5-br-k8s-pod,K8s log pod bind business rules>>) when the webhook reports a HOST entity but the log path encodes a pod name under `/mnt/spark/logs/<pod>/`.
...
Cluster problems intentionally keep **HOST** as Dynatrace **`dt.source_entity`** (in the Spark Observability MZ). ServiceNow **`K8sLogPodCiBind`** rebinds alert CI to **`cmdb_ci_kubernetes_pod`** from the path segment.
