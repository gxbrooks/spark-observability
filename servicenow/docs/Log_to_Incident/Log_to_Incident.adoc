...
===== Processors
Processing sets **`spark.mode`** (`Cluster` or `Client`) from the log path prefix, then mode-specific fields. Canonical specification: `observability/dynatrace/tenants/pdt20158/integrations/spark-openpipeline-log-alerts-pipeline.json.j2`.
...
|spark.pod
|pod name (e.g. `spark-master-0`)
|unset

|spark.instance
|unset
|`<host.name>:<driver-instance>`
...
|dt.source_entity
|HOST (in Spark Observability MZ; pod rebind in ServiceNow)
|same CUSTOM_DEVICE as `spark.device`
|===
      ...
      "processors": [
        {
          ...
          "matcher": "isNull(spark.client.instance) AND matchesValue(spark.log.path, \"/mnt/spark/client-logs/*\")",
          "dql": {
            "script": "parse spark.log.path, \"LD '/mnt/spark/client-logs/' LD:spark.client.instance LD '/spark-app'\" | fieldsAdd spark.application = \"spark-client\""
          }
        },
        {
          ...
          "matcher": "isNotNull(spark.client.instance) OR matchesValue(spark.log.path, \"/mnt/spark/client-logs/*\") OR matchesValue(log.source, \"/mnt/spark/client-logs/*\")",
          "fieldsAdd": {
            "fields": [
        ...
        },
        {
          ...
          "matcher": "isNull(k8s.pod_name) AND isNotNull(spark.log.path) AND spark.application != \"spark-client\" AND NOT matchesValue(spark.log.path, \"/mnt/spark/client-logs/*\")",
          "dql": {
        ...
        },
        {
          ...
          "matcher": "isNull(k8s.pod_name) AND spark.application != \"spark-client\" AND isNull(spark.client.instance) AND (matchesValue(log.source, \"/mnt/spark/logs/*\")) AND NOT matchesValue(log.source, \"/mnt/spark/client-logs/*\")",
          "dql": {
        ...
        },
        {
          ...
          "type": "drop",
          "enabled": true,
          "description": "Exclude legacy host-chapter log directories (replaced by /mnt/spark/client-logs/)",
...
While setting 'dt.davis.is_merging_allowed=false' prevents Davis from merging events across pods or clients, we still want related duplicate lines within one driver/pod to refresh the same problem. **`event.unique_identifier`** includes the Log4j short class and line (`{spark.log.class}:{spark.log.line}`) parsed from the log content, so distinct call sites open distinct problems without flooding on every identical repeating line.
...
* Client Davis processor sets **`dt.davis.is_merging_allowed=false`** and **`event.unique_identifier={host.name}:{spark.driver.instance}|{spark.log.class}:{spark.log.line}`** so concurrent drivers stay separate and distinct Log4j class:line sites do not collapse into one problem (ServiceNow still maps all client paths to Application Service **Spark Client**).
* Cluster Davis processor likewise sets **`dt.davis.is_merging_allowed=false`** and **`event.unique_identifier={spark.pod}|{spark.log.class}:{spark.log.line}`** so different pods and different class:line sites do not merge into one problem.
* ServiceNow **`ResolveApplicationService.applySparkClientAlertBinding()`** binds alert CI to **Spark Client** when `/client-logs/` is present.
...
|`spark.mode` |Custom log source / OpenPipeline |`Client` or `Cluster`

|`spark.pod` |Cluster path segment |Must equal `cmdb_ci_kubernetes_pod.name`
...
|`event.name` |OpenPipeline Davis |`Client application log … on {spark.instance}` or `Cluster application log … on {spark.pod}`
