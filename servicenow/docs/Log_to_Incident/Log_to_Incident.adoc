...
// asciidoc.preview.asciidoctorAttributes (Asciidoctor.js cannot run Rouge).
...
The end-to-end flow in <<fig-log-to-incident-end-to-end-data-flow>> must handle both the **Service-side** and the **Client-side** use cases. The CSDM model for the application will differ between a client and the appliation service. Where the log files reside will be different between a client application and the application service itself. Lastly, the way in which the right CI is mapped to an incident will differ, too. As such, Steps 1, 2, and 6 must handle both the Service-side and the Client-side use cases. 
...
==== Revaluate the workstation-host use case
...
<<fig-spark-servicenow-model-client-side>> illustrates the Client-side model of the Spark Client. It shows a device (and host) independent model consistting of a single Application Service CI that represents the Spark Client.
...
Now, given the Spark Client application service how do we map a log message on the client device to the Spark Client application service? This is done by creating a custom device entity in Dynatrace that represents the Spark Client. This custome device entity must then be included in any log problems sent as events to ServiceNow from Dynatrace's OneClient agent on the device to Dynatrace's OpenPipeline.
...
Apache Spark is a Java application that uses Log4j2 for logging. We need to configure Log4j2 to emit log lines with the requisite metadata.
...
|Pod label |`servicenow.io/application-service-identifier=<identifier>` matching a CSDM Application Service (Step 1)
...
==== Dynatrace and ServiceNow (Step 2)
...
[#step-2-dynatrace]
=== Step 3 — Dynatrace detects ERROR and WARN log lines
...
=== Step 4 — Dynatrace sends the problem to ServiceNow
...
=== Step 5 — ServiceNow receives the event and binds infrastructure CI
...
At Step 5 the **Observability engineer** typically performs **verification**, not new Dynatrace or application configuration:
...
. If **`cmdb_ci`** is a Linux server instead of a pod, return to <<step-2-dynatrace,Step 3 — Dynatrace>> and verify **`k8s.pod_name`** extraction and **`lookupEntity(CLOUD_APPLICATION_INSTANCE, k8s.pod_name)`** in the OpenPipeline pipeline (<<spec-dt-openpipeline,OpenPipeline specification>>).
...
**No ServiceNow-side changes** are required on the monitored **application** (the Java/Spark/Python workload). **No additional Dynatrace** configuration is required at Step 5 if Steps <<step-3-dynatrace,Step 3 — Dynatrace>> and Step 3 are correct.

ServiceNow cannot bind to **`cmdb_ci_kubernetes_pod`** unless <<step-3-dynatrace,Step 3 — Dynatrace>> sets **`dt.source_entity`** to the pod entity and Step 0 imported that entity into **`sys_object_source`**.

---

=== Step 6 — Create incident bound to Application Service
...
. `K8sLogPodCiBind` sets `em_event.cmdb_ci` and `em_alert.cmdb_ci` to the **pod CI** whose `name` matches the log path segment (Step 5).
