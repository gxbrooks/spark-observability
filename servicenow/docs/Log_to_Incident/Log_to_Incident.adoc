...
This chapter walks the Log to Incident reference path for **Spark Client Mode** only: application logs under `/mnt/spark/client-logs/`, OneAgent custom log source file tail, OpenPipeline Client enrichment (`spark.as_identifier`), and the Client Davis processor. Cluster Mode (container stdout) is <<ch-cluster-mode,Chapter 6>>.
...
- We need an Application Service bind key (`spark.as_identifier`) for ServiceNow.
...
|`spark.as_identifier`
|Custom log source
|`spark-client` (Application Service bind key for ServiceNow)
|===
...
|spark.as_identifier
|`spark-client` (from custom source / pipeline; SN AS bind)

|dt.source_entity
|HOST (OneAgent file tail; in MZ). SN binds via `spark.as_identifier`, not HOST
|===
...
. **`ensure-client-as-identifier`** — sets `spark.as_identifier=spark-client` when missing so ServiceNow can bind the Application Service.
...
|`event.description`
|includes `{spark.log.class}:{spark.log.line}`, path, and `spark.as_identifier=` for SN bind
    ...
    "rankedEvents": [
      {
        ...
        "eventDescription": "Spark Client critical ERROR error at TaskSetManager:270 in /mnt/spark/client-logs/par-a/spark-app.log spark.event_kind=CRITICAL_LOG_EVENT spark.as_identifier=spark-client: 2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job"
      }
    ],
    "startTime": 1719502426000,
    "endTime": -1
  },
  "ProblemDetailsText": "Spark Client critical ERROR error at TaskSetManager:270 in /mnt/spark/client-logs/par-a/spark-app.log spark.event_kind=CRITICAL_LOG_EVENT spark.as_identifier=spark-client:\n2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job\n\n1 impacted entity: Lab3",
  "ImpactedEntities": [
    {
      "entityId": "HOST-A1B2C3D4E5F67890",
      "name": "Lab3",
...
. After a test problem fires, confirm **`em_event.source=SGO-Dynatrace`** and **`em_event.cmdb_ci`** is populated (Application Service via `spark.pod_identifier` for Cluster; Spark Client AS via `spark.as_identifier` for Client).
...
. Deploy or verify the generic entity bind business rules (<<step-5-br-k8s-pod,entity bind business rules>>) so `ResolveApplicationService` can bind from `spark.as_identifier` / `spark.pod_identifier`.
...
Cluster Mode prefers OneAgent Log-module entity context (CAI/PGI) on Container Output; ServiceNow still binds Application Service via **`spark.pod_identifier`** (pod CI → `svc_ci_assoc`), not HOST file-tail.
...
.Incident correlation patterns — alert CI vs Application Service
...
.CSDM identifier lookup fields on Application Service
[cols="30%,35%,35%",options="header"]
|===
...
.Verify Spark Client Application Service operational status
[source,bash]
----
cd ansible
...
|Service-Side: `svc_ci_assoc` membership from pod CI to Application Service (tag-based Service Mapping). Spark client: `resolveFromSparkClientLogPath()` when alert text contains `/client-logs/` → Application Service **Spark Client**
...
|`spark.as_identifier` |Client custom source / OpenPipeline |Application Service `identifier` (`spark-client`) for SN bind
...
|`event.description` |OpenPipeline Davis |includes path plus `spark.as_identifier=` or `spark.pod_identifier=` and `{content}`
|===
...
NOTE: Cluster/Client Davis problems keep OneAgent **HOST** as `dt.source_entity` inside the Spark Observability management zone. ServiceNow **`ResolveApplicationService`** binds Cluster alerts via **`spark.pod_identifier`** (pod CI → `svc_ci_assoc` AS) and Client alerts via **`spark.as_identifier`** to Application Service **Spark Client**, ignoring HOST for those paths.
...
|SM membership
|svc_ci_assoc: pod CI is a member of Application Service Myapp Worker Zone A (tag-based Service Mapping)

|Incident
|incident.cmdb_ci → Application Service Myapp Worker Zone A (via Service-Side svc_ci_assoc lookup)
|===
