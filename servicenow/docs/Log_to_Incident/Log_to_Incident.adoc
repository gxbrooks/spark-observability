...
Client Mode configuration is <<ch-client-mode,Chapter 5>>; Cluster Mode (stdout / Container Output) is <<ch-cluster-mode,Chapter 6>>. ServiceNow Event Management remains <<Phase 4 — ServiceNow,Phase 4>>.
...
[[ch-client-mode]]
== Chapter 5 — Client Mode Use Case

This chapter walks the Log to Incident reference path for **Spark Client Mode** only: application logs under `/mnt/spark/client-logs/`, OneAgent custom log source file tail, OpenPipeline Client enrichment (`spark.service_instance`), and the Client Davis processor. Cluster Mode (container stdout) is <<ch-cluster-mode,Chapter 6>>.
...
To start with, <<list-spark-log-file-sample>> illustrates a sample log file from a Client Mode Spark Driver.
...
|`log.source`
|OneAgent
|Registered path under `/mnt/spark/client-logs/…`
...
|Custom log source `${1}`
|Driver directory under `/mnt/spark/client-logs/` (`par-a`, PID, …)

|`spark.service_instance`
|Custom log source
|`Spark-Client` (service instance bind key for ServiceNow)
|===
...
This rule routes Client file tails (`/mnt/spark/client-logs/*`) and Cluster Container Output (see <<ch-cluster-mode,Chapter 6>>) to the shared Spark log processing OpenPipeline. This chapter covers the Client path only.
...
. **Davis** - where the log entries are matched to a Davis processor to generate a Davis event that is ultimately convertert to a problem and forwarded to ServiceNow.
...
The Client-relevant OpenPipeline processors are defined in <<lst-openpipeline-processing>>, below:
...
(Cluster Mode adds `enrich-cluster-from-k8s` on Container Output; see <<ch-cluster-mode,Chapter 6>>.)

[[lst-openpipeline-processing]]
.Spark OpenPipeline processing processors
[source,json] 
----
{
    "processing": {
...
. the event's timestamp being with the 'dt.davis.event_timeout' window of the existing Davis problem
...
Furthermore, we assume that the same error signature across pods and/or client instances does *not* necessarily have the same root cause. For example, in a enterprise getting the same error around the same time across clusters in different regions likely does not have the same root cause. Granted, having the same error signature across pods in the same region likely does have the same root cause. Also, we assume that different error signatures within the same pod or client do no necessarily have the same root cause. However, Davis *does not* have the fidelity to define event by root cause and differentiate between different remote or local clusters or clients.
...
The Davis process define the following properties across all three processors:
...
The Davis processors are defined in <<lst-openpipeline-davis>>, below.

[[lst-openpipeline-davis]]
.Spark OpenPipeline Davis processors
[source,json] 
----
{
    "davis": {
...
Note that alerting profiles management zone is set to the Spark Observability management zone. This is important as the alerting profile will now filter out any problems that do not have an `dt.source_entity` in the Spark Observability management zone. This means that the impacted entities (hosts for OneAgent file tails and CPU) must be in the Spark Observability management zone, too.
...
The template in <<spec-dt-webhook-payload-json,Problem notification payload specification>> is substituted by Dynatrace at send time. **`ProblemDetailsJSON.rankedEvents[]`** carries the OpenPipeline Davis properties from <<lst-openpipeline-davis,OpenPipeline Davis processors>> (`event.description`, entity ids, severity). Those ranked events are the **problem specification** that flows into ServiceNow **`em_event.description`** (via **`ProblemDetailsText`**) and into **`em_event.additional_info`**.
...
[[ch-cluster-mode]]
== Chapter 6 — Cluster Mode Use Case

This chapter mirrors <<ch-client-mode,Chapter 5>> for **Spark Cluster Mode**: Log4j2 **Console** → container stdout, DynaKube `logMonitoring: {}` → `log.source=Container Output`, OpenPipeline enrichment from `k8s.pod.name` (`spark.mode=Cluster`, `spark.pod_identifier`), and the Cluster Davis processor. There is **no** custom log source path on `/mnt/spark/logs` for application logs.
...
<<list-spark-log-file-sample-cluster>> illustrates a sample Cluster Mode log line (same Log4j pattern as Client Mode).

[.figure-indent]
[[list-spark-log-file-sample-cluster]]
.Spark Cluster Mode log sample (stdout)
[source,text]
----
2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job
----
...
NOTE: A separate RollingFile under `/opt/spark/logs` may still exist for the OTel EventEmitter path; that file is **not** used for L2I Davis alerts.
...
.Log4j Console — Cluster Mode (container stdout)
[source,properties]
----
...
Cluster Container Output shares the Spark Lab OpenPipeline with Client file tails (same routing object as <<lst-spark-log-routing,Chapter 5 routing>>). Matcher includes `log.source` = `Container Output`.

[[lst-spark-log-routing-cluster]]
.Spark OpenPipeline logs routing (Cluster-relevant matcher)
[source,json]
----
{
  "schemaId": "builtin:openpipeline.logs.routing",
    ...
    "davis": {
      "processors": [
        ...
        },
        {
          ...
          "matcher": "(loglevel == \"WARN\" OR loglevel == \"ERROR\") AND isNull(spark.mode) AND matchesValue(log.source, \"Container Output\")",
          "davis": {
            "properties": [
              { "key": "event.type", "value": "ERROR_EVENT" },
              { "key": "spark.event_kind", "value": "CRITICAL_LOG_EVENT" },
...
Once a Davis problem is created it is forwarded to ServiceNow via the same alerting profile / webhook as Chapter 5 (<<lst-alerting-profile>>).

===== Storage

Storage is the same default_logs bucket assignment as Chapter 5 (<<lst-openpipeline-storage>>).
...
Payload shape matches <<spec-dt-webhook-payload-json,Chapter 5 payload specification>>. Cluster Mode example:
...
. If Cluster **`cmdb_ci`** remains empty or a Linux server, return to <<step-2-dynatrace-cluster,Chapter 6 — OneAgent>> / <<lst-openpipeline-processing-cluster,Cluster OpenPipeline processors>> and verify **`spark.pod_identifier`** from `k8s.pod.name` on Container Output, then the entity bind rules.
...
**No ServiceNow-side changes** are required on the monitored **application** (the Java/Spark/Python workload). **No additional Dynatrace** configuration is required at Phase 5 if <<ch-client-mode,Chapter 5>> and <<ch-cluster-mode,Chapter 6>> Dynatrace phases are correct.
