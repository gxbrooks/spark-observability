...
Route both Cluster (`/mnt/spark/logs/*`) and Client (`/mnt/spark/client-logs/*`) paths to the Spark log processing OpenPipeline.
...
===== Processors
OneAgent custom log source already sets `spark.*` variables prior to entry of the pipeline. In addition to the standard Dynatrace fields for `content`, `loglevel`, and `log.source`, the available fields on entry to the pipeline processors are: 
...
The pipeline only derives fields that cannot be set at ingest (compose `spark.instance`, bind Client CUSTOM_DEVICE, parse Log4j class:line from `content`). Canonical specification: `observability/dynatrace/tenants/pdt20158/integrations/spark-openpipeline-log-alerts-pipeline.json.j2`.
...
Once the metadata fields are defined in the processors, we are ready to create Dynatrace problems for those log entries with a log level of WARN or ERROR. There is one Client matcher and one Cluster matcher; both use the same **`event.name`** / **`event.unique_identifier`** shape keyed by **`{spark.mode}|{spark.log.class}:{spark.log.line}`**. **`spark.mode`** is definitive — Davis matchers do not re-check path patterns. Lines without a parsed class:line are skipped (no Davis event). Missing **`spark.mode`** enrichment surfaces as a separate fail-visible Davis event. The current implementation is shown below.
...
===== How Dynatrace groups and refreshes log-derived problems

Two different mechanisms are easy to conflate:

**1. Event report identity (create vs refresh an event)** — Dynatrace links OpenPipeline Davis *reports* using **`event.name` + `event.type` + `dt.source_entity`**. Matching reports **refresh** the same Davis event (and keep it open for `dt.davis.event_timeout`, default 15m). A non-matching triple **opens a new** event. This is *dedup / refresh*, not problem merge.

**2. Problem merge (bundle distinct events into one problem)** — Separate Davis events may still be **merged into one problem** when merge is allowed, their active windows overlap, and they share the same (or closely related) source entity / Smartscape neighborhood. **`dt.davis.is_merging_allowed`** controls that second step only.

**`event.unique_identifier`** is a property carried on the event (and useful for ServiceNow class:line extraction and for Classic log-event split recipes). For OpenPipeline Davis reports it does **not** replace the `event.name` + `event.type` + `dt.source_entity` identity key. Putting class:line only in `event.unique_identifier` while leaving `event.name` as “Client application log WARN on lab3:par-a” caused every WARN on that driver/host to refresh **one** event — so DQL joins keyed by class:line looked like “most lines have no Problem.”

Lab design now:

* **`event.name`** = `{spark.mode} error at {spark.log.class}:{spark.log.line}` so each mode|class:line is its own event identity (on a given source entity). Same class:line from `par-a` and `par-b` therefore **shares one event** (true cross-driver grouping) without needing problem merge.
* **`event.unique_identifier`** = `{spark.mode}|{spark.log.class}:{spark.log.line}` (aligned with that grouping).
* **`dt.davis.is_merging_allowed=false`** so *different* class:line events (and Client vs Cluster) are **not** bundled into one problem on the shared HOST. Stress testing with merge enabled immediately recreated false Client↔Cluster / multi-class:line bundles (e.g. `Master:250` with `TaskSchedulerImpl:244`).

Repeating the same WARN/ERROR at the same class:line within the timeout **refreshes** one event/problem rather than opening a new problem per line.

After the problems are created, Dynatrace stores the *log records* in the default_logs bucket (storage processors below). Davis events/problems live in Grail event/problem stores.
