...
This chapter walks the Log to Incident reference path for **both** Spark modes that share one OpenPipeline (`spark-lab-log-alerts`), one alerting profile / SGO webhook, and the same ServiceNow Event Management automation in <<Phase 4 — ServiceNow,Phase 4>>:
...
|`spark.mode` / `spark.pod_identifier`
...
- We need a service instance bind key (`spark.service_instance`) for ServiceNow.
      ...
      "values-and-enrichment": [
        {
          ...
          "enrichment": [
            ...
            { "type": "attribute", "key": "spark.service_instance", "value": "Spark-Client" }
          ]
        },
        {
          "path": "/mnt/spark/client-logs/*/spark-app-*.log",
...
On entry from Container Output, Kubernetes enrichment supplies `k8s.pod.name`. The Cluster processor sets `spark.mode` and `spark.pod_identifier` before Log4j parse / Davis.
...
|dt.source_entity
|OneAgent Log module entity context (CAI/PGI when associated). SN binds via `spark.pod_identifier`, not HOST file-tail
|===

Cluster-relevant processors:

. **`enrich-cluster-from-k8s`** — Container Output in namespace `spark` with `k8s.pod.name` matching `spark-*` → `spark.mode=Cluster` and `spark.pod_identifier=toString(k8s.pod.name)`.
...
`spark.pod_identifier` carries the Kubernetes pod name for the ServiceNow `svc_ci_assoc` membership lookup to the service instance. OpenPipeline does **not** bake CAI entity ids at apply time.
...
|dt.source_entity
|HOST (OneAgent file tail; in MZ). SN binds via `spark.service_instance`, not HOST
|===
...
. **`ensure-client-service-instance`** — sets `spark.service_instance=Spark-Client` when missing so ServiceNow can bind the service instance.
    ...
    "processing": {
      "processors": [
        ...
        },
        {
          ...
          "matcher": "spark.mode == \"Client\" AND isNull(spark.service_instance)",
          "fieldsAdd": {
            "fields": [
              { "name": "spark.service_instance", "value": "Spark-Client" }
            ]
          }
        },
        {
...
`spark.service_instance` carries the K8s-sanitized service instance display name (`Spark-Client` for **Spark Client**) that ServiceNow uses to bind the Client service instance; Davis leaves `dt.source_entity` as the OneAgent HOST.
...
|`event.description`
|includes class:line, `spark.pod_identifier=`, and `{content}` for SN bind

|`dt.source_entity`
|Log-module entity context when present (CAI/PGI). SN prefers `spark.pod_identifier` over HOST
      ...
      "processors": [
        {
          ...
          "davis": {
            "properties": [
              ...
              {
                "key": "event.description",
                "value": "Spark Cluster critical {loglevel} error at {spark.log.class}:{spark.log.line} on {spark.pod_identifier} spark.event_kind=CRITICAL_LOG_EVENT spark.pod_identifier={spark.pod_identifier}: {content}"
              },
              { "key": "dt.davis.is_merging_allowed", "value": "false" },
...
|`event.description`
|includes `{spark.log.class}:{spark.log.line}`, path, and `spark.service_instance=` for SN bind
...
|`pipeline`
|`spark-lab-log-alerts` (OpenPipeline customId; stamped in processing)
      ...
      "processors": [
        {
          ...
          "type": "davis",
          "enabled": true,
          "description": "Client WARN/ERROR → ERROR_EVENT (DT enum); spark.event_kind=CRITICAL_LOG_EVENT; stamp spark.service_instance; leave HOST as dt.source_entity",
          ...
          "davis": {
            "properties": [
              ...
              {
                "key": "event.description",
                "value": "Spark Client critical {loglevel} error at {spark.log.class}:{spark.log.line} in {spark.log.path} spark.event_kind=CRITICAL_LOG_EVENT spark.service_instance={spark.service_instance}: {content}"
              },
              { "key": "dt.davis.is_merging_allowed", "value": "false" },
              { "key": "spark.service_instance", "value": "{spark.service_instance}" },
    ...
    "rankedEvents": [
      {
        ...
        "eventDescription": "Spark Cluster critical ERROR error at TaskSetManager:270 on spark-master-0 spark.event_kind=CRITICAL_LOG_EVENT spark.pod_identifier=spark-master-0: 2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job"
      }
    ],
    "startTime": 1719502426000,
    "endTime": -1
  },
  "ProblemDetailsText": "Spark Cluster critical ERROR error at TaskSetManager:270 on spark-master-0 spark.event_kind=CRITICAL_LOG_EVENT spark.pod_identifier=spark-master-0:\n2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job\n\n1 impacted entity: spark-master-0",
  "ImpactedEntities": [
    {
      "entityId": "CLOUD_APPLICATION_INSTANCE-A1B2C3D4E5F67890",
      "name": "spark-master-0",
...
ServiceNow **`ResolveApplicationService`** binds via `spark.pod_identifier` → pod CI by name → `svc_ci_assoc` service instance membership (ignores HOST when the pod identifier is present).
    ...
    "rankedEvents": [
      {
        ...
        "severityLevel": "ERROR",
        "title": "Spark critical ERROR error at TaskSetManager:270",
        "eventDescription": "Spark Client critical ERROR error at TaskSetManager:270 in /mnt/spark/client-logs/par-a/spark-app.log spark.event_kind=CRITICAL_LOG_EVENT spark.service_instance=Spark-Client: 2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job"
      }
    ],
    "startTime": 1719502426000,
    "endTime": -1
  },
  "ProblemDetailsText": "Spark Client critical ERROR error at TaskSetManager:270 in /mnt/spark/client-logs/par-a/spark-app.log spark.event_kind=CRITICAL_LOG_EVENT spark.service_instance=Spark-Client:\n2026-07-20 12:40:29 ERROR TaskSetManager:270 - Task 0 in stage 0.0 failed 4 times; aborting job\n\n1 impacted entity: Lab3",
  "ImpactedEntities": [
    {
      "entityId": "HOST-A1B2C3D4E5F67890",
      "name": "Lab3",
...
. After a test problem fires, confirm **`em_event.source=SGO-Dynatrace`** and **`em_event.cmdb_ci`** is populated (service instance via `spark.pod_identifier` for Cluster; Spark Client AS via `spark.service_instance` for Client).
. If Cluster **`cmdb_ci`** remains empty or a Linux server, return to <<step-2-dynatrace-cluster,Phase 2 — OneAgent (Cluster)>> / <<lst-openpipeline-processing-cluster,Cluster OpenPipeline processors>> and verify **`spark.pod_identifier`** from `k8s.pod.name` on Container Output, then the entity bind rules.
...
. Deploy or verify the generic entity bind business rules (<<step-5-br-k8s-pod,entity bind business rules>>) so `ResolveApplicationService` can bind from `spark.service_instance` / `spark.pod_identifier`.
...
Cluster Mode prefers OneAgent Log-module entity context (CAI/PGI) on Container Output; ServiceNow still binds service instance via **`spark.pod_identifier`** (pod CI → `svc_ci_assoc`), not HOST file-tail.
...
| CSDM display name **Spark Client** → stamp `spark.service_instance=Spark-Client`
...
* Custom log source sets **`spark.mode`**, **`spark.driver.instance`** or **`spark.pod_identifier`**, **`spark.service_instance`** (Client), and **`spark.log.path`**. OpenPipeline parses **`spark.log.class` / `spark.log.line`**.
...
|`spark.pod_identifier` |Cluster path segment |Must equal `cmdb_ci_kubernetes_pod.name`

|`spark.instance` |Client OpenPipeline |`<host.name>:<driver-instance>` under `/mnt/spark/client-logs/`

|`spark.service_instance` |Client custom source / OpenPipeline |K8s-sanitized service instance display name (`Spark-Client`) for SN bind
...
|`event.description` |OpenPipeline Davis |includes path plus `spark.service_instance=` or `spark.pod_identifier=` and `{content}`
|===
...
NOTE: Cluster/Client Davis problems keep OneAgent **HOST** as `dt.source_entity` inside the Spark Observability management zone. ServiceNow **`ResolveApplicationService`** binds Cluster alerts via **`spark.pod_identifier`** (pod CI → `svc_ci_assoc` AS) and Client alerts via **`spark.service_instance`** to service instance **Spark Client**, ignoring HOST for those paths.
