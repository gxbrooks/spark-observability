      ...
      "values-and-enrichment": [
        ...
        },
        {
          ...
          "enrichment": [
            ...
            { "type": "attribute", "key": "spark.pod_name", "value": "${1}" }
          ]
        },
        {
          "path": "/mnt/spark/logs/*/spark-app-*.log",
...
|spark.device
|unset
|CUSTOM_DEVICE id for Spark Client

|dt.source_entity
|HOST (OneAgent file tail; in Spark Observability MZ)
|HOST (OneAgent file tail; in MZ). `spark.device` carries CUSTOM_DEVICE id for SN
|===
...
. **`resolve-client-device`** - sets the `spark.device` field to the CUSTOM_DEVICE id for the Spark Client.
    ...
    "processing": {
      "processors": [
        ...
        },
        {
          ...
          "type": "fieldsAdd",
          "enabled": true,
          "description": "Client: set spark.device (CUSTOM_DEVICE id) for ServiceNow; leave OneAgent HOST as dt.source_entity",
          "matcher": "spark.mode == \"Client\"",
          "fieldsAdd": {
            "fields": [
              { "name": "spark.device", "value": "{{ dt_spark_client_custom_device_entity_id | mandatory }}" }
            ]
          }
        },
        {
...
The custom device provides a Dynatrace entity ID that ServiceNow will map to the pseudo Application Service for the Spark Client.
...
|`event.type`
|is always `ERROR_EVENT` 

|`event.name`
|`Critical log.level error at {spark.log.class}:{spark.log.line}`

|`event.description`
|`{spark.mode} critical log.level error at {spark.log.class}:{spark.log.line}`
...
----
{
    "davis": {
      "processors": [
        {
          "id": "spark-warn-error-davis",
          "type": "davis",
          "enabled": true,
          "description": "Client/Cluster ERROR/WARN: one Davis event/problem per class:line (merge off)",
          "matcher": "(loglevel == \"WARN\" OR loglevel == \"ERROR\") AND (spark.mode == \"Client\" OR spark.mode == \"Cluster\") AND isNotNull(spark.log.class) AND isNotNull(spark.log.line)",
          "davis": {
            "properties": [
              { "key": "event.type", "value": "ERROR_EVENT" },
              { "key": "event.name", "value": "Critical log.level error at {spark.log.class}:{spark.log.line}" },
              {
                "key": "event.description",
                "value": "{spark.mode} critical log.level error at {spark.log.class}:{spark.log.line}"
              },
              { "key": "dt.source_entity", "value": "{dt.source_entity}" },
              { "key": "event.unique_identifier", "value": "{spark.log.class}:{spark.log.line}" },
...
. If Cluster **`cmdb_ci`** remains a Linux server instead of a pod, return to <<step-2-dynatrace,Phase 3 — Dynatrace>> and verify **`spark.pod_name`** enrichment in the custom log source (<<spec-dt-openpipeline,OpenPipeline specification>>), then the **K8s log pod bind** rules.
...
* Custom log source sets **`spark.mode`**, **`spark.driver.instance`** or **`spark.pod_name`**, and **`spark.log.path`**. OpenPipeline parses **`spark.log.class` / `spark.log.line`**.
...
|`spark.pod_name` |Cluster path segment |Must equal `cmdb_ci_kubernetes_pod.name`

|`spark.instance` |Client OpenPipeline |`<host.name>:<driver-instance>` under `/mnt/spark/client-logs/`

|`spark.device` |Client OpenPipeline |CUSTOM_DEVICE Spark Client entity id

|`event.name` |OpenPipeline Davis |`{spark.mode} error at {spark.log.class}:{spark.log.line}`

|`event.unique_identifier` |OpenPipeline Davis |`{spark.mode}|{spark.log.class}:{spark.log.line}`

|`event.description` |OpenPipeline Davis |`Client|Cluster {loglevel} on {spark.log.path}: {content}`
|===
...
NOTE: Cluster problems stay on **HOST** entities inside the Spark Observability management zone (pods are not in the MZ). ServiceNow **`K8sLogPodCiBind`** rebinds Cluster alerts to the **pod CI** from the log path; **`ResolveApplicationService`** binds Client alerts via `/client-logs/` (or Davis `Client error at …`) to Application Service **Spark Client**.
