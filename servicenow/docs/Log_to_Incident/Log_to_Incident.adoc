...
**Log to Incident** is the Observability process that detects and turns *critical* application log messages into *tracked* IT incidents so the application operations team can respond in time. It does this through a three broad phase process: 
...
. An application writes operational messages to its application log (errors, warnings, or other agreed severity).
. Observability tooling collects those messages and decides which ones are severe enough to treat as a *problem* (a detected issue worth escalating).
. The problem is handed to IT service management as an *event* or *alert* tied to the right infrastructure or application identity.
. Automation creates (or updates) an *incident* assigned to the application’s support organization, carrying enough log context for diagnosis.
. The operations team works and closes the incident through normal ITSM practice.

The process intentionally does **not** depend on a single observability product or a single ITSM product. What must stay stable is the *contract*: which log lines matter, how the application is identified for assignment, and what context travels with the incident.

=== What must be agreed up front

Not every log line should become an incident. Flooding operations with low-value tickets increases noise and lengthens real outages. Observability engineers **must** agree with application developers and operations on:

* Which severities or message patterns are *incident-worthy*.
* How log lines are structured so the incident carries actionable context (what failed, where, which workload or component).
* Which *application service* (or equivalent business CI) owns the work when an incident is created.

=== Client and server applications

Many systems have both *service-side* components (clustered services, pods, long-running servers) and *client-side* components (drivers, desktops, mobile apps, or other unmanaged endpoints). Log to Incident must still escalate both, even when the client is not a managed CI in the CMDB. That usually means:
...
Later chapters describe how to realize Log to Incident on specific platforms. The first implementation in this document is **Using ServiceNow and Dynatrace**. Additional chapters (for example Elastic Stack and ServiceNow) **may** be added without renaming the process.

== Using ServiceNow and Dynatrace

This chapter is the reference implementation of Log to Incident using **Dynatrace** as the observability platform and **ServiceNow** as the ITSM / Event Management platform. Apache Spark is the sample application (WARN and ERROR log lines escalate in the lab; production agreements often limit escalation to ERROR after review with operations and development).
...
* https://spark.apache.org/docs/latest/index.html[Apache Spark Documentation]

=== End-to-end flow
The figure below illustrates the end-to-end flow of this implementation: the application JVM writes to the application log file; Dynatrace detects, correlates, and notifies; ServiceNow creates the event, alert, and incident.
...
Critical to the process is selecting the best configuration items (CIs) to assign to the alert and even more importantly the incident. CI selection is especially important for the incident as the incident will be directed to the support group of the CI. Care must be taken to make sure that the CI then represents the application and the support group on the application CI the application's operations team. 
...
The end-to-end flow in Figure XXX must handle both the **Service-side** and the **Client-side** use cases. The CSDM model for the application will differ between a client and the appliation service. Where the log files reside will be different between a client application and the application service. Lastly, the way in which the right CI is mapped to an incident will differ, too. As such, Steps 0, 1, and 5 must handle both the Service-side and the Client-side use cases. 

For those not famialr with Spark's client-server model, the figure below illustrates the different components of the Spark application.

image::spark-architecture.svg[Spark client-server architecture — Master, Workers, client driver, and History Server,width=75%,align=center]

Spark can run in one of two modes: *Client Mode* or *Cluster Mode*.
In client mode, the client-side application is a standalone application that runs on a workstation or host.The client then communicates with the Spark Master running as a service and the executors running on the Spark Workers. The Spark Master and the workers run on the service side. The executors on the workers run jobs and constitue the heart of the application. 

In cluster mode, the Spark Driver runs on a Kubernetes pod and communicates with the Spark Master running as a service and the executors running on the Spark Workers. The Spark Master and the workers run on the service side. The executors on the workers run jobs and constitue the heart of the application. 


Clean up the sections below:

Typically, the client-side application is not managed by ServiceNow's CMDB. Also, it does not run on a Kubernetes pod. Therefore, the CI that is mapped to the incident will be a manual logical service that is defined in the CSDM model. The log file will be located in a fixed path segment on the host. The way in which the right CI is mapped to an incident will be done by the application developer by setting the `incident.cmdb_ci` field on the alert and incident records.

Both patterns require the fully formatted log line in event, alert, and incident descriptions; Dynatrace **ERROR** or **WARN** detection; and **`incident.cmdb_ci`** = `cmdb_ci_service_discovered` (Application Service), not the infrastructure CI on the alert.

Both patterns follow the same observability path from application log line to ServiceNow incident. Figure 1 is the navigation map; pattern-specific configuration is called out in Steps 0, 1, and 5.

Steps 2–4 are shared; Steps 0, 1, and 5 include dual **Service-side** and **Client-side** subsections.

=== Step 0 — ServiceNow Modeling

Before log problems can bind to infrastructure CIs and incidents to an **Application Service**, we must model the application in ServiceNow. This involves definiing the CSDM hierarchy and disovering pattern-specific CMDB components. We need to do this for the Service-side and the Client-side components. 

==== ServiceNow Modeling

The ServiceNow modeling is the data foundation of the Log to Incident process. It consists of the CSDM hierarchy and the pattern-specific CMDB components. We need to do this for the Service-side and the Client-side components. 
...
|Dynatrace entity merged via `sys_object_source`
|===
...
|May appear on **`em_alert.cmdb_ci`** when Davis binds to HOST; **not** used for incident CI

|Per-driver `<instance>` segment
|Disambiguates concurrent drivers; **not** used for AS lookup

|Optional `cmdb_ci_appl` on in-scope hosts
